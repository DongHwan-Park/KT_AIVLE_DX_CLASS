...
df_line = df.loc[df['구분'] == '하차']
st.dataframe(df_line)

st.text("3. '연번','날짜','역번호','역명','구분','합계' 제외하고 저장 & 확인")
df_line = df_line[df_line.columns.difference(['날짜','연번','역번호','역명','구분','합계'])]
st.dataframe(df_line)

st.text("4. 아래 방법으로 데이터프레임 변환하여 저장 & 확인")
st.caption("melt 함수 사용 unpivot: identifier-'호선', unpivot column-'시간', variable column-'인원수'")
# id_vars 기준 , var_name='시간' 으로 unpivot 풀다
